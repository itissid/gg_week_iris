...
class SimpleNeuralNet:
    ...
    def __init__(
            ...
            activation='relu', init_biases_as_zero=True,
    ):
    ...
    # forward function
    def forward(self, X, is_training=True, dropout_p=0, temp=1):
        ...
        # output activation
        self.output = self.softmax(self.output_input, temp=temp)
        return self.output
    ...
    # train
    def train(self, X_train, y_train, epochs, dropout_p=0, decay_rate=1, temp=1):
        for epoch in range(epochs):
            ...
            # Forward pass
            output = self.forward(
                X_train_shuffled, is_training=True, dropout_p=dropout_p, temp=temp,
            )
    ...
    def predict(self, X, temp=1):
        return np.argmax(self.forward(X, is_training=False, temp=temp), axis=1)
